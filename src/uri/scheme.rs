...
#[derive(Clone)]
pub struct Scheme {
    pub(super) inner: SchemeInner,
}

#[derive(Clone, Debug)]
pub(super) enum SchemeInner<T = Box<ByteStr>> {
    None,
    Standard(Protocol),
    Other(T),
}
...
impl Scheme {
    ...
    pub const HTTP: Scheme = Scheme {
        inner: SchemeInner::Standard(Protocol::Http),
    };

    /// HTTP protocol over TLS.
    pub const HTTPS: Scheme = Scheme {
        inner: SchemeInner::Standard(Protocol::Https),
    };

    pub(super) fn empty() -> Self {
        Scheme {
            inner: SchemeInner::None,
        }
    }
...
impl<'a> TryFrom<&'a [u8]> for Scheme {
    ...
    fn try_from(s: &'a [u8]) -> Result<Self, Self::Error> {
        ...
        match SchemeInner::parse_exact(s)? {
...
impl Hash for Scheme {
        ...
        H: Hasher,
    {
        match self.inner {
            SchemeInner::None => (),
            SchemeInner::Standard(Protocol::Http) => state.write_u8(1),
            SchemeInner::Standard(Protocol::Https) => state.write_u8(2),
            SchemeInner::Other(ref other) => {
...
impl<T> SchemeInner<T> {
    pub(super) fn is_none(&self) -> bool {
        matches!(*self, SchemeInner::None)
    }
}
...
impl SchemeInner<usize> {
    // Postcondition: On all Ok() returns, s is valid UTF-8
    fn parse_exact(s: &[u8]) -> Result<SchemeInner<()>, InvalidUri> {
        match s {
            ...
            _ => {
                ...
                Ok(SchemeInner::Other(()))
            }
        }
    }

    pub(super) fn parse(s: &[u8]) -> Result<SchemeInner<usize>, InvalidUri> {
        if s.len() >= 7 {
            // Check for HTTP
...
impl<T> From<Protocol> for SchemeInner<T> {
    fn from(src: Protocol) -> Self {
        SchemeInner::Standard(src)
    }
}

#[doc(hidden)]
impl From<SchemeInner> for Scheme {
    fn from(src: SchemeInner) -> Self {
        Scheme { inner: src }
    }
}
